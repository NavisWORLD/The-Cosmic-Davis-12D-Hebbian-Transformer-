...
class SelfAwarenessBootstrap:
    ...
    async def store_in_memory(
        ...
        if memory_system is None:
            try:
                # Try cosmosynapse-native path first, then cosmos
                try:
                    from cosmos.memory.memory_system import MemorySystem
                except ImportError:
                    logger.warning("MemorySystem not available (standalone mode)")
                    return None
        ...
        try:
            ...
            if learning_engine is None:
                try:
                    ...
                    logger.info("Swarm learning not available (standalone CosmoSynapse mode)")
                    learning_engine = None
            ...
            if learning_engine:
                ...
                try:
                    ...
                    logger.info("FitnessTracker not available (standalone CosmoSynapse mode)")
                    return False
...
async def awaken(
    ...
    Usage:
        from cosmosynapse.consciousness.self_awareness import awaken
...
def awaken_sync(
    ...
    Usage:
        from cosmosynapse.consciousness.self_awareness import awaken_sync
