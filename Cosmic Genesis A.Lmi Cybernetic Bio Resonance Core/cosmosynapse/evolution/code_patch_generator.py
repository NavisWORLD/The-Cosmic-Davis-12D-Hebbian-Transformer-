...
class CodePatchGenerator:
    ...
    def generate_code_upgrade(
        ...
        """
        Generate a patch to modify actual Python source code.
        """
        # Validate syntax before creating patch
        if not self.validate_python_syntax(new_code):
            logger.error("Proposed code upgrade has syntax errors")
            return None

        patch = CodePatch(
            patch_id=self._generate_patch_id(new_code),
            patch_type="function",
            target_file=file_path,
            target_section="full_file" if original_code == "FULL_FILE" else "partial",
            original_content=original_code,
            proposed_content=new_code,
            rationale=reason,
            fitness_improvement=confidence
        )
        
        self.patches.append(patch)
        self._save_history()
        logger.info(f"Generated code upgrade for {file_path}: {patch.patch_id}")
        return patch
    ...
    def apply_patch(self, patch: CodePatch, auto_test: bool = True) -> bool:
        ...
        try:
            ...
            elif patch.patch_type == "function":
                # DIRECT CODE MODIFICATION
                target_path = self.project_root / patch.target_file
                if not target_path.exists():
                    raise FileNotFoundError(f"Target file not found: {patch.target_file}")
                
                # Check if exact replacement or search/replace
                current_text = target_path.read_text(encoding='utf-8')
                
                if patch.original_content == "FULL_FILE":
                    new_text = patch.proposed_content
                else:
                    if patch.original_content not in current_text:
                        raise ValueError("Original content not found in target file")
                    new_text = current_text.replace(patch.original_content, patch.proposed_content)
                
                # Verify syntax of FULL FILE after patching
                if target_path.suffix == ".py" and not self.validate_python_syntax(new_text):
                    raise SyntaxError("Patch results in invalid Python syntax")
