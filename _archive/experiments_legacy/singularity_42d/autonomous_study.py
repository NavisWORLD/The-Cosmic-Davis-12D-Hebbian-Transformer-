...
sys.path.append(str(Path(__file__).parents[2] / "audio_12d")) # Add audio_12d path 
    ...
    try:
        while True:
            ...
            for _, content in reader.read_stream():
                ...
                # Process Multimodal Input (Feel the text)
                # We treat the first 500 chars as the "immediate perception" for emotion
                ...
                log_progress(log_file, f"Emotion: {emotion.classify_emotion()} (Valence: {emotion.valence:.2f}, Arousal: {emotion.arousal:.2f})")
                if thought:
                    log_progress(log_file, f"Thought: {thought}")
                
                # Train 12D (with emotional modulation)
                res_12d = trainer_12d.train_on_text(content, gen, seq_len=128, batch_size=16, learning_rate=current_lr)
                loss_12d = res_12d['loss']
                
                # Train 42D (with emotional modulation)
                res_42d = trainer_42d.train_on_text(content, gen, seq_len=128, batch_size=16, learning_rate=current_lr)
                ...
                # Get actual token stream - show words being processed
                import json
                words = content.split()
                # Take a random sample of 30-50 words to show what's being learned right now
                sample_size = min(40, len(words))
                if sample_size > 10:
                    start_idx = random.randint(0, max(0, len(words) - sample_size))
