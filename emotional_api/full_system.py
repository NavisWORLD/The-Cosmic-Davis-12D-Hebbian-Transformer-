    ...
    def process_frame(self, frame):
        ...
        if self.tracker:
            ...
            if result['detected'] and result['blendshapes']:
                ...
                # Update API with real data - FIXED to prevent simulation overwrite
            ...
            phase = calculate_geometric_phase(upper, lower)
            self.api.update_from_tensors(upper, lower, phase)
            self.face_detected = True # Assume detected in fallback for now, or change logic
            
            # Check Sleep Condition (No face for > X time)
            # Using 30 seconds for test; Production should be 30 mins
        ...
        async def websocket_endpoint(websocket: WebSocket):
            ...
            try:
                ...
                while True:
                    try:
                        packet = controller.get_current_packet()
                        # print(f"DEBUG: Calculated packet keys: {packet.keys()}") 
                        await websocket.send_json({
                            "type": "cosmos_packet",
                            "face_detected": controller.face_detected,
                            **packet
                ...
                print(f"❌ Client disconnected. Total: {len(controller.connected_clients)}")
            except Exception as e:
